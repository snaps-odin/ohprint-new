...
export default class CSQNAItem extends React.Component {
	constructor(...args) {
		super(...args);
	...
	onClickDelete(event) {
		let { actions: { handleOpenAlert, handleDeleteQNAByBoardId }, value: { boardId } } = this.props;
		let editable = this.getEditableInfo();
	...
	onClickRegistry(event) {
		let { actions: { handleCreateQNAByBoardIdSeq, handleCreateQNAByBoardId, handleOpenAlert }, value: { boardId }, pop } = this.props;
		let editable = this.getEditableInfo();
  ...
  requestQNADelete() {
	...
	resizeHeight() {
		...
		if (pop) {
			handleResizeHeight(getHeight(this.scope))
		} else {
	...
	render() {
		let { states, value } = this.props;
		let { config: { url: { cdn: cdnUrl } } } = states;
		let { boardId, list } = value;
		let { edit } = this.state;

		let editable = list ? this.getEditableInfo() : {};
		let titleWidth = Math.max(0, getWidth(this.scope) - (16 * 2 + 55));

		return (
			<div className="item" data-board-id={boardId} ref={(c) => {this.scope = c;}}>
				<div className="top">
					<span>
						<span className={`status ${value[ 'qnaStatusCode' ] === '336003' ? 'replied' : ''}`}>
							{value[ 'qnaStatusName' ]}
						</span>
						<span className="title" style={{ width: `${titleWidth}px` }}>{value[ 'title' ]}</span>
					</span>
					<span>
						<span className={"category"}>{value[ 'categoryGroupName' ]}{`>`}{value[ 'categoryName' ]}</span>
						<span className="date">{toDate(value[ 'registrationDate' ], 'YYYY/MM/DD')}</span>
					</span>
				</div>
				<ul className="middle">
							...
							<span className="field">
								...
								<span className="bottom">
									<span>
										...
										<button type="button"
										        className="btn-black"
										        onClick={this.onClickRegistry}>{editable[ 'enable' ] ? '확인' : '등록하기'}</button>
									</span>
								</span>
							</span>
						)
