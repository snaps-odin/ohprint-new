...
`;
class OPM395 extends React.Component {
  constructor(props) {
    super(props);
  ...
  onMapClick(area, idx){
    const { handleOnReceiveMessage, states : { currentForm : { values } } } = this.props;
    let { desc } = area;

    if(desc.type === "marketingAgree"){

      if(values){
        if(values["marketing-agree"]){
  ...
  render() {
    return (
      <Wrap>
        <ImageMap
          className="usage-map"
          src={`https://cdn.ohprint.me/Upload/Data1/event/202302/OPM395/pc/web@2x.jpg`}
          map={this.state.imageMapData}
          onMapClick={this.onMapClick}
        />
        <Marketing>
        <Field name="marketing-agree"
               label={(
                 <span className="desc">마케팅 수신에 동의합니다. (이메일)</span>
               )}
