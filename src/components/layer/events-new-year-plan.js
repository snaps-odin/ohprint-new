...
class EventsNewYearWishes extends React.Component {
  ...
  onSubmit(values) {
    ...
    let { handleOpenAlertLayer, handleOpenContentsLayer, handleCloseContentsLayer, handleCreateEventsCommentsByIdx, handleCreateCouponByCouponCode } = actions;
    ...
    let couponNumber = '129CF47F-47F4-7A09';
    ...
    return handleCreateEventsCommentsByIdx(eventIdx, params).then(async result => {
      ...
      if (msg.indexOf('이미 응모에 참여하였습니다') !== -1) {
        ...
        handleOpenContentsLayer(LayerTypes.EVENTS_NEW_YEAR_PLAN_SUCCESS, {
          callback: () => {
            Promise.all([
              callback && callback()
            ]).then(() => {
  ...
  render() {
    ...
    return (
      <div className={`events-new-year-plan-recommend ${showType ? 'events-policy' : ''} popup`}
        ...
        <div className="container" style={showType ? { display: 'none', height: '0' } : null}>
          <form onSubmit={handleSubmit(this.onSubmit)}>
                ...
                <table border={0}>
                  ...
                  <tr>
                    <th  className={"caption"} colSpan={"2"}>
                      2023년, 새해 계획을 남겨주세요.<br/>
                      오프린트미가 당신의 계획을 응원할게요!
                    </th>
                  </tr>
                  <tr>
                    <td colSpan="2">
                      ...
                      <div className={"subCaption"}>
                        * 이벤트 종료 후 남겨주신 계획은 오프린트미 이벤트 및 컨텐츠로 활용 될 수 있습니다. (추후 협의 예정)
                      </div>
                    </td>
                  </tr>
                  </tbody>
            ...
            <div className="bottom" ref={(c) => {this.bottom = c;}}>
              <button type="submit"
                      className={`btn-${valid ? 'black' : 'gray'}-medium`}>완료
              </button>
            </div>
          </form>
        </div>
          ...
          <div className="container">
            <form>
              ...
              <div className="middle">
                <section>
                  <div>
                    (주)위블링 (오프린트미) 는 개인정보 관련 법률 (개인정보보호법, 정보통신망
                    이용촉진 및 정보보호에 관한 법률)을 준수하며 수집된 개인 정보는
                    해당 서비스 외에 다른 용도로 절대 사용하지 않으며, 사용자 요청 시
                    바로 삭제할 수 있습니다.
                  </div>
                </section>
                <section>
